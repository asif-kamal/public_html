...
var lab6_2aBtn_state = 0;
var lab6_2bBtn_state = 0;
...
$(document).ready(function () {
    ...
    //place event in the click function and no e in preventDefault()
    $("#lab6_2a").click((event) => {
        if (lab6_2aBtn_state == 0) {
            lab6_2aBtn_state++;
            $("#lab6_2a").html("Reset");
            event.preventDefault();
            var formData = { 'Size': $('input[name=mSize]').val() }; // json format
            if (formData.Size == "") {
                lab6_2aBtn_state = 0;
                $("#lab6_2a").html("Run");
                return
            ...
            $.ajax({
                type: 'GET',
                url: 'php/runLab6_2a.php',
                async: true,
                data: formData,
                dataType: 'html',
                encode: true
            }).always(function (data) { // log data to the console so we can see
                console.log(data); // here we will handle errors and validation messages
                $("#results6_2a").html(data); // location.reload();
            });
            event.preventDefault();
        } else {
            lab6_2aBtn_state = 0;
    ...
    $("#lab6_2b").click((event) => {
        if (lab6_2bBtn_state == 0) {
            lab6_2bBtn_state++;
            $("#lab6_2b").html("Reset");
            event.preventDefault();
            var formData = {
                'Size': $('input[name=nSize]').val(),
                'Repetitions': $('input[name=nRepetitions]').val()
            }; // json format
            if (formData.Size == "" || formData.Repetitions == "") {
                alert("Please enter ALL fields then click Run");
                lab6_2bBtn_state = 0;
                $("#lab6_2b").html("Run");
                return
            }
            $("#results6_2b").html("<i class='fas fa-spinner fa-spin'></i>");
            $.ajax({
                type: 'GET',
                url: 'php/runLab6_2b.php',
                async: true,
                data: formData,
                dataType: 'html',
                encode: true
                ...
                console.log(data); // here we will handle errors and validation messages
                $("#results6_2b").html(data); // location.reload();
            });
            event.preventDefault();
        } else {
            lab6_2bBtn_state = 0;
